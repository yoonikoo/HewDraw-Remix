...
unsafe fn arsene_summon_desmummon(boma: &mut BattleObjectModuleAccessor) {
    ...
    if boma.is_status(*FIGHTER_STATUS_KIND_ATTACK_AIR) {
        if ControlModule::check_button_on_trriger(boma, *CONTROL_PAD_BUTTON_GUARD) {
            ...
            WorkModule::on_flag(
                boma,
                *FIGHTER_JACK_INSTANCE_WORK_ID_FLAG_RESERVE_SUMMON_DISPATCH,
            );
...
unsafe fn arsene_grappling_hook(
...
unsafe fn aerial_grappling_hook_stall(
    boma: &mut BattleObjectModuleAccessor,
    motion_kind: u64,
    frame: f32,
) {
...
unsafe fn rebel_gauge_update(boma: &mut BattleObjectModuleAccessor) {
    let damage_dealt = VarModule::get_float(
        boma.object(),
        vars::common::instance::LAST_ATTACK_DAMAGE_DEALT,
    );

    if damage_dealt == 0.0 || WorkModule::is_flag(boma, *FIGHTER_JACK_INSTANCE_WORK_ID_FLAG_DOYLE) {
        return;
    }

    let meter_gain =
        ParamModule::get_float(boma.object(), ParamType::Agent, "rebel_gauge.add_gauge_mul");

    app::FighterSpecializer_Jack::add_rebel_gauge(
        boma,
        app::FighterEntryID(boma.get_int(*FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID)),
        meter_gain * damage_dealt,
    );
