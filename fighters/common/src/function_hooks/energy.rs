...
unsafe fn update(energy: &mut FighterKineticEnergyControl, boma: &mut BattleObjectModuleAccessor) {
    ...
    let accel_diff = match reset_type {
        FallAdjust | FallAdjustNoCap | FlyAdjust | ShootDash | ShootBackDash | RevolveSlashAir
        | MoveGround | MoveAir => accel_add_x * stick.x.signum() + stick.x * energy.accel_mul_x,
        WallJump => {
        ...
        Dash | DashBack => loop {
            ...
            let keep_frame = WorkModule::get_param_int(
                boma,
                smash::hash40("common"),
                smash::hash40("dash_speed_keep_frame"),
            );
                ...
                || reset_type == DashBack
            {
                ...
                if WorkModule::get_int(
                    boma,
                    *FIGHTER_STATUS_DASH_WORK_INT_TURN_DASH_FROM_DASH_COUNT,
                ) < keep_frame
                {
                    energy.speed_max.x = 0.0;
                    energy.speed_brake.x = 0.0;
        ...
        TurnRun => {
            ...
            let mut brake = WorkModule::get_param_float(boma, smash::hash40("ground_brake"), 0)
                * WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("run_brake_brake_mul"),
                );

            let ground_module =
                *(boma as *const BattleObjectModuleAccessor as *const u64).add(0x88 / 0x8);
            let some_float = *(ground_module as *const f32).add(0x130 / 0x8);
            if some_float * energy.lr <= -0.1 {
                let turn_run_brake = WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("turn_run_stop_brake_mul"),
                );
                mul *= turn_run_brake;
        ...
        ItemLift => loop {
            ...
            let stick_rate =
                WorkModule::get_float(boma, *FIGHTER_STATUS_ITEM_LIFT_WORK_FLOAT_STICK_RATE);
            energy.speed_max.x *= stick_rate;
            break (accel_add_x * stick.x.signum() + stick.x * energy.accel_mul_x) * stick_rate;
        },
        Swim => {
            let speed_mul = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("swim_speed_mul"),
            );
        ...
        SwimDrown => {
            let speed_mul = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("swim_drown_speed_x_mul"),
            ) * WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("swim_speed_mul"),
        ...
        Turn | TurnNoStop | TurnNoStopAir => {
            if reset_type == TurnNoStop || reset_type == TurnNoStopAir {
                if (!TurnModule::is_turn(boma) || energy.accel_mul_x == 0.0)
                    && energy.speed.x == 0.0
                {
                    energy.enable = false;
                    return;
            ...
            (accel_add_x * stick.x.signum() + stick.x * energy.accel_mul_x)
                * WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("turn_speed_mul"),
                )
        }
        Ladder => {
            let ladder_y = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("ladder_stick_y"),
            );
            let (speed_max, accel_y) = if ladder_y <= stick.y.abs() {
                if stick.y <= 0.0 {
                    let down_max = WorkModule::get_param_float(
                        boma,
                        smash::hash40("common"),
                        smash::hash40("ladder_speed_d_max"),
                    );
                    // lerp the down_max
                    let down_max = ((stick.y.abs() - ladder_y) / (1.0 - ladder_y)) * down_max;
                    let attack_mul = WorkModule::get_param_float(
                        boma,
                        smash::hash40("common"),
                        smash::hash40("ladder_attack_speed_mul"),
                    );
                    (down_max * attack_mul, -down_max * attack_mul)
                } else {
                    let up_max = WorkModule::get_param_float(
                        boma,
                        smash::hash40("common"),
                        smash::hash40("ladder_speed_d_max"),
                    );
    ...
    if do_standard_accel {
        ...
        if energy.unk[1] != 0 {
            if !(((energy._x9c != 0.0
                && (stick.x <= 0.0
                    || energy._xa0 <= 0.0
                    || energy.speed_max.x.abs() <= energy._x9c.abs()))
                && (stick.x >= 0.0
                    || energy._xa0 >= 0.0
                    || energy.speed_max.x.abs() <= energy._x9c.abs()))
                && ((stick.x <= 0.0 || 0.0 <= energy._xa0)
                    && (0.0 <= stick.x || energy._xa0 <= 0.0)))
            {
    ...
    if !*(status_module as *const bool).add(0x12a) {
        if StatusModule::situation_kind(boma) == *SITUATION_KIND_AIR {
            let horizontal_limit = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("common_air_speed_x_limit"),
            );
            let vertical_limit = if energy.speed.y <= 0.0 {
                WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("air_speed_down_limit"),
                )
            } else {
                WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("air_speed_up_limit"),
                )
            };
            ...
            let speed_limit = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("ground_speed_limit"),
            );
...
unsafe fn initialize(
    energy: &mut FighterKineticEnergyControl,
    ...
    match reset_type {
        FallAdjust | FallAdjustNoCap | StopCeil | WallJump => {
            let mut stable_speed =
                WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0);
            if reset_type == StopCeil {
                stable_speed *= WorkModule::get_param_float(
                    boma,
                    smash::hash40("common"),
                    smash::hash40("stop_ceil_speed_x_stable_mul"),
                );
            }
            ...
            let air_x_speed_max =
                    ...
                    && energy.unk[2] == 0
                {
                    WorkModule::get_param_float(
                        boma,
                        smash::hash40("common"),
                        smash::hash40("air_speed_x_limit"),
                    )
                } else {
                    -1.0
                };
            ...
            energy.accel_mul_x =
                WorkModule::get_param_float(boma, smash::hash40("air_accel_x_mul"), 0);
            energy.accel_add_x =
                WorkModule::get_param_float(boma, smash::hash40("air_accel_x_add"), 0);
        }
        FlyAdjust => {
        ...
        Dash | TurnRun | DashBack => {
            ...
            energy.speed_brake = PaddedVec2::new(brake, 0.0);
            energy.accel_mul_x =
                WorkModule::get_param_float(boma, smash::hash40("run_accel_mul"), 0);
            energy.accel_add_x =
                WorkModule::get_param_float(boma, smash::hash40("run_accel_add"), 0);
        }
        ...
        RevolveSlashAir => {
            let speed_max =
                WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0)
                    * WorkModule::get_param_float(
                        boma,
                        smash::hash40("param_special_hi"),
                        smash::hash40("rslash_air_max_x_mul"),
                    );
            ...
            energy.accel_mul_x = WorkModule::get_param_float(
                boma,
                smash::hash40("param_special_hi"),
                smash::hash40("rslash_air_max_x_mul"),
            );
        ...
        Turn | TurnNoStop => {
            ...
            energy.speed_brake = PaddedVec2::new(brake, 0.0);
            energy.accel_mul_x =
                WorkModule::get_param_float(boma, smash::hash40("walk_accel_mul"), 0);
            energy.accel_add_x =
                WorkModule::get_param_float(boma, smash::hash40("walk_accel_add"), 0);
        }
        Free => {
            let speed_max =
                WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0);
        ...
        ItemLift => {
            ...
            energy.speed_brake = PaddedVec2::new(brake, 0.0);
            energy.accel_mul_x =
                scale * WorkModule::get_param_float(boma, smash::hash40("item_lift_accel_mul"), 0);
            energy.accel_add_x =
                scale * WorkModule::get_param_float(boma, smash::hash40("item_lift_accel_add"), 0);
        }
        Swim => {
            energy.speed_brake = PaddedVec2::new(
                WorkModule::get_param_float(
        ...
        Ladder => {
            let up_speed = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("ladder_speed_u_max"),
            );
            let down_speed = WorkModule::get_param_float(
                boma,
                smash::hash40("common"),
                smash::hash40("ladder_speed_d_max"),
            );
...
unsafe fn setup(
    energy: &mut FighterKineticEnergyControl,
    reset_type: EnergyControllerResetType,
    initial_speed: &Vector3f,
    unk: u64,
    boma: &mut BattleObjectModuleAccessor,
) {
    energy.clear_energy();
    ...
    match reset_type {
        FallAdjust | FallAdjustNoCap | StopCeil | WallJump => {
            energy.unk[2] =
                if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_JUMP_NO_LIMIT_ONCE) {
                    1
                } else {
                    0
                };
        ...
        ShootDash => {
            energy.speed.x = if 0.0 >= energy.speed.x * energy.lr {
                -energy.lr
                    * WorkModule::get_param_float(boma, smash::hash40("shoot_dash_speed_f"), 0)
            } else {
                energy.speed.x
                    - energy.lr
                        * WorkModule::get_param_float(boma, smash::hash40("shoot_dash_speed_f"), 0)
        ...
        ShootBackDash => {
            energy.speed.x = if 0.0 <= energy.speed.x * energy.lr {
                energy.lr
                    * WorkModule::get_param_float(boma, smash::hash40("shoot_dash_speed_b"), 0)
            } else {
                energy.speed.x
                    + energy.lr
                        * WorkModule::get_param_float(boma, smash::hash40("shoot_dash_speed_b"), 0)
        ...
        RevolveSlashAir => {
            energy.speed.x *=
                WorkModule::get_param_float(boma, smash::hash40("rslash_air_spd_x_mul"), 0);
        }
        Free => {
            energy.speed = PaddedVec2::zeros();
        }
        MoveGround => {
            let new_speed = utils::ext::energy::KineticEnergy::adjust_speed_for_ground_normal(
                &energy.speed,
                boma,
            );
...
pub fn install() {
    skyline::install_hooks!(update, initialize, setup);
}
